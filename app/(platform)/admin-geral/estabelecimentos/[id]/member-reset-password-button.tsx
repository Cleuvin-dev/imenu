...
import {
  ...
  type ResetMemberPasswordActionState,
} from "@/app/(platform)/admin-geral/estabelecimentos/[id]/actions";

const initialState: ResetMemberPasswordActionState = {};
...
export function MemberResetPasswordButton({ establishmentId, userId }: { establishmentId: string; userId: string }) {
  const boundAction = resetMemberPasswordAction.bind(null, establishmentId, userId);
  ...
  if (state.temporaryPassword) {
    return (
      <div className="flex flex-col gap-1 rounded-control border border-primary-200 bg-primary-50 px-2 py-1.5 text-xs text-primary-900">
        <p>Nova senha (repasse por canal seguro):</p>
