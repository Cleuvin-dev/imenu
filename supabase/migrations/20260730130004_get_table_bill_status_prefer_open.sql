-- Fase 7 — correção: get_table_bill_status escolhia a sessão mais recente
-- só por `opened_at desc`, sem critério de desempate. Numa mesa com duas
-- sessões criadas na mesma transação (ex.: teste de integração, onde
-- now() fica congelado por toda a transação) ou com timestamps muito
-- próximos, a ordenação podia escolher a sessão ERRADA (uma já fechada em
-- vez da aberta). Corrigido para sempre preferir a sessão `open`, se
-- existir, independentemente do timestamp — semântica mais correta de
-- qualquer forma: uma sessão aberta é sempre "mais atual" para o
-- consumidor do que qualquer sessão já fechada.
