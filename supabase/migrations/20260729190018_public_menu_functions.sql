-- Fase 4 — RPCs públicas de leitura limitada (docs/07 §11).
-- Nenhuma delas expõe tabelas internas diretamente; ambas validam tudo
-- internamente e nunca revelam se um slug/token existe quando inválido
-- (docs/03 RF-PUB-001/014, docs/11 AC-PUB-002).
...
-- ensure_guest_session: cria/renova a sessão anônima da mesa a partir do
-- hash do token guardado no cookie. Nunca recebe nem devolve o token bruto.
...
begin
  if not exists (
    ...
    raise exception 'ensure_guest_session: mesa inválida' using errcode = '22023';
  end if;

  update public.guest_sessions
    set last_seen_at = now(), expires_at = p_expires_at
  ...
  if v_session_id is null then
    insert into public.guest_sessions (establishment_id, table_id, token_hash, expires_at)
    values (p_establishment_id, p_table_id, p_token_hash, p_expires_at)
    returning id into v_session_id;
  end if;
...
revoke all on function public.ensure_guest_session(uuid, uuid, text, timestamptz) from public, authenticated;
grant execute on function public.ensure_guest_session(uuid, uuid, text, timestamptz) to anon;
